...
MainController::MainController() {
  ...
  data->window = SDL_CreateWindow("Pull Wall", SDL_WINDOWPOS_CENTERED,
...
void MainController::clear() {
  int score = 550 - data->elapsedSteps * 20 - data->wallMovedTimes * 15;
  ...
  SDL_Surface *spriteImage2 =
      TTF_RenderText_Solid(data->font, text2, (score < 0 ? red : green));
  SDL_Texture *spriteTex2 =
      SDL_CreateTextureFromSurface(data->renderer, spriteImage2);
...
void MainController::updateDisplay() {
  ...
  // Draw walls
  {
    ...
    SDL_Texture *wallTexBuf =
        SDL_CreateTextureFromSurface(data->renderer, data->wallTex);
    for (int i = 0; i < 9; i += 1) {
      if (i % 2 == 0) {
        for (int j = 0; j < 9; j += 1) { // Vertical walls
          if (data->walls[i][j]) {
            ...
            wallRect.w = 80;
            wallRect.h = 10;
            SDL_RenderCopyEx(data->renderer, wallTexBuf, NULL, &wallRect, 90,
                             NULL, SDL_FLIP_NONE);
          }
        }
      } else {
  ...
  // Draw goal
  {
    ...
    SDL_Point points[] = {{15, 15}, {75, 15}, {75, 75}, {15, 75},
                          {15, 15}, {75, 75}, {15, 75}, {75, 15}};
  ...
  // Draw player
  {
    ...
    SDL_Texture *playerTexBuf =
        SDL_CreateTextureFromSurface(data->renderer, data->playerTex);
    switch (data->playerDir) {
    case Direction::Up:
      SDL_RenderCopyEx(data->renderer, playerTexBuf, NULL, &playerRect, 0, NULL,
                       SDL_FLIP_NONE);
      break;
    case Direction::Right:
      SDL_RenderCopyEx(data->renderer, playerTexBuf, NULL, &playerRect, 90,
                       NULL, SDL_FLIP_NONE);
      break;
    case Direction::Down:
      SDL_RenderCopyEx(data->renderer, playerTexBuf, NULL, &playerRect, 180,
                       NULL, SDL_FLIP_NONE);
      break;
    case Direction::Left:
      SDL_RenderCopyEx(data->renderer, playerTexBuf, NULL, &playerRect, 270,
                       NULL, SDL_FLIP_NONE);
      break;
    }
  ...
  // Draw socre
  SDL_Color green = {0x00, 0xff, 0x00};
  char const *text = ("Step: " + std::to_string(data->elapsedSteps) +
                      " , Moved: " + std::to_string(data->wallMovedTimes))
                         .c_str();
  SDL_Surface *stepImage = TTF_RenderText_Solid(data->font, text, green);
  SDL_Texture *stepTex =
      SDL_CreateTextureFromSurface(data->renderer, stepImage);
